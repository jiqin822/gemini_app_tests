...
const BreathingOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col items-center justify-center text-white animate-fade-in relative overflow-hidden">
       {/* Grid Background */}
...
const AppreciationOverlay = ({ onClose, onSubmit, partnerName }: { onClose: () => void, onSubmit: (items: string[]) => void, partnerName: string }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/95 flex flex-col items-center justify-center text-white animate-fade-in p-6">
...
export const TherapistMode: React.FC<Props> = ({ user, onExit, onAddNotification }) => {
      ...
      setMessages([
        {
          id: '1',
          role: 'system',
          text: `**SESSION INITIALIZED**\n\nTarget Subject: **${selectedSubject.name}**\nRelation: ${selectedSubject.relationship}\n\nState your current status or conflict.`,
          timestamp: now
        }
      ]);
    }
  ...
  if (!selectedSubject) {
    return (
      <div className="flex flex-col h-full bg-slate-50 relative animate-fade-in font-sans">
        {/* Background Grid */}
        <div className="absolute inset-0 z-0 pointer-events-none opacity-20" 
            style={{ 
        ...
        <div className="flex-1 p-6 overflow-y-auto relative z-10">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 border-b border-slate-200 pb-2">Select Target Subject</h2>
          ...
          >
              ...
              >
                  ...
                  <span className="text-[9px] font-mono font-bold uppercase text-slate-400 px-1">
                      {msg.role === 'user' ? 'YOU' : msg.role === 'system' ? 'SYSTEM' : 'AI MODEL'}
                  </span>
