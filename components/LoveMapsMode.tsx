                    ...
                    <div className="animate-fade-in pb-20">
                         ...
                         <div className="space-y-6">
                             {OPEN_ENDED_QUESTIONS.filter(q => myAnswers[q.id]).map((q, i) => (
                                 <div key={q.id} className="bg-white border-2 border-slate-200 p-4 shadow-sm group hover:border-slate-400 transition-colors">
                                     <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center gap-2">
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest border border-slate-200 px-1.5 py-0.5">{q.category}</span>
                                        </div>
                                        <Edit3 size={14} className="text-slate-300" />
                                     </div>
                                     <h4 className="font-bold text-slate-900 text-sm leading-tight mb-3">{q.text}</h4>
                                     
                                     <textarea 
                                        value={myAnswers[q.id]}
                                        onChange={(e) => setMyAnswers(prev => ({ ...prev, [q.id]: e.target.value }))}
                                        className="w-full bg-slate-50 border-2 border-slate-100 p-3 text-sm font-medium text-slate-700 focus:outline-none focus:bg-white focus:border-indigo-500 transition-all resize-y min-h-[80px]"
                                     />

                                     <div className="mt-2 text-right">
                                         <button 
                                            onClick={() => handleSaveSpec(q.id, myAnswers[q.id])}
